<template>
  <div class="leadrecip pdt">
    ...
    <div class="lead-ser flex-box middle">
      <input class="lead-ser-inp flex" type="text" v-model="keyword" placeholder="请填写名称首字母搜索">
...
export default {
  ...
  created: function () {
    ...
    this.cloudIndex = this.$route.params.index
    console.log(this.cloudIndex)
  },
  methods: {
    // 搜索
    searchTap: function () {
    ...
    chooseTap: function (sec) {
      this.$store.commit('recipientList', sec)
      this.$store.commit('recipient', sec)
      let param = {}
      param.index = this.cloudIndex
      param.item = sec
      this.$store.commit('recipientCloudList', param)
      this.$router.go(-1)
    }
  }
}
