<template>
  <div class="balance pdt">
    ...
    <div class="cloudlog-nav flex-box middle white">
      <p class="active flex">
        <span @click="sortNavTap(null)" :class="{'active':otype==null}">所有行为</span>
      </p>
      <p class="flex">
      ...
      <p class="flex">
        <span @click="sortNavTap(2)" :class="{'active':otype==2}">参与活动</span>
      </p>
      <p class="flex">
        <span @click="sortNavTap(3)" :class="{'active':otype==3}">后台调整</span>
      </p>
    </div>
        ...
        <ul class="integ-ul">
          <li v-for="(item, index) in userlist" :key="item.id" class="integ-li white">
            <div class="integ-li-top flex-box middle between">
              ...
              <p v-if="item.otype==2" class="state">参与活动</p>
              <p v-if="item.otype==3" class="state">后台调整</p>
            </div>
            <div class="integ-li-desc">
              <p>来源：{{item.comefrom}}</p>
              <div class="flex-box">
                <p class="flex">积分：<span>{{item.use_integral}}</span></p>
                <span>所剩积分：<span>{{item.integral}}</span></span>
              </div>
              <p>备注：{{item.remark}}</p>
            </div>
...
export default {
  ...
  methods: {
    ...
    // 获取积分日志
    getList: function () {
      this.axios.post(this.api.get_integral_log, {
