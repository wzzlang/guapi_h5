<template>
  <div class="pick pdt pbb">
    ...
    <div class="pick-top white">
      ...
      <p>如果您不想将商品放入云仓，而是想要直接帮您发出。您可以通过商场直发，快速下单，由我们给您配货，打包并发货，足不出户，帮您完成一笔又一笔代购生意</p>
    </div>
    <div class="pick-item white">
      ...
      <div class="pick-cont">
        ...
        <div v-if="sender" class="pick-sec flex-box">
    ...
    <div class="pick-item white">
      ...
      <div class="pick-cont">
        <div class="pick-sec flex-box">
          ...
          <p v-if="recipient" class="pick-sec-p flex">{{recipient.realname}}</p>
        </div>
        <div class="pick-sec flex-box">
          <p class="pick-sec-tit">收件人手机</p>
          <p v-if="recipient" class="pick-sec-p flex">{{recipient.mobile}}</p>
        </div>
        <div class="pick-sec flex-box">
          <p class="pick-sec-tit">收件人地址</p>
          <p v-if="recipient" class="pick-sec-p flex">{{recipient.area}} {{recipient.address}}</p>
        </div>
      </div>
    </div>
    <div class="pick-item white">
    ...
    <div class="pick-item white">
      ...
      <div class="pick-cont">
        <div v-if="list.parces" class="pick-parcel">
    ...
    <div class="pick-item white">
      <div v-if="list.parces" class="pick-cont">
    ...
    <div class="pick-item white">
      ...
      <div class="pick-cont">
        ...
        <div v-if="list.parces" class="pick-sec flex-box">
          <p class="pick-sec-tit">转运费用</p>
          <p class="pick-sec-p flex">${{list.parces.total_fee}}</p>
        </div>
        <div v-if="list.parces" class="pick-sec flex-box">
...
export default {
  ...
  created: function () {
    ...
    this.axios.post(this.api.save_mall_good_info, params).then(res => {
      console.log(res)
      if (res.data.code === '200') {
        ...
        if (!this.$store.state.picker) {
          this.$store.commit('picker', this.list.sender)
        }
        if (!this.$store.state.recipient) {
          this.$store.commit('recipient', this.list.receipt)
        }
        this.sender = this.$store.state.picker // 取件人
        this.recipient = this.$store.state.recipient // 收件人
        // this.$set(this.recipient, 0, this.$store.state.recipient)
      } else {
  ...
  methods: {
    ...
    submitTap: function () {
      ...
      this.axios.post(this.api.commit_mall, {
    ...
    splitDragTap: function () {
      ...
      this.$router.push({path: '/SplitDrag'})
    }
  }
}
</script>
