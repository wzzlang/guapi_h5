  ...
  <div class="pick pdt pbb">
      ...
      <div class="pick-cont">
        <div class="pick-parcel">
          <div v-for="(item, index) in list.parces" :key="index" class="pick-parcel-item">
            ...
            <div class="flex-box">
              <p class="flex">预估重量：{{item.weight/1000}}KG</p>
    ...
    <div class="pick-item white">
      ...
      <div class="pick-cont">
        <div class="pick-sec flex-box">
          <p class="pick-sec-tit">转运费用</p>
          <p class="pick-sec-p flex">${{list.total_fee | toFixed}}</p>
        </div>
        <div class="pick-sec flex-box">
        ...
        <div class="pick-sec flex-box">
          ...
          <p class="pick-sec-p flex">{{isInteg ? '$'+isIntegDed : '没有选择积分抵扣'}}</p>
        </div>
        <div class="border"></div>
        <div class="pick-sec flex-box">
...
export default {
  ...
  created: function () {
    ...
    console.log(this.parcesParams)
    this.getSplitDrag()
  },
  methods: {
    // 获取包裹拆分数据
    getSplitDrag: function () {
      this.axios.post(this.api.auto_opera_cloud_infos,
        {formdata: JSON.stringify(this.parcesParams)}).then(res => {
        console.log(res)
    ...
    submitTap: function () {
      ...
      let formdata = {
        ...
        total: parseFloat(this.totalPrice) + parseFloat(this.list.total_fee), // 总计
        handle_price: this.payMoney,
        pay_fee: this.payType === 0 ? this.list.wxcharge : this.list.apipaycharge,
