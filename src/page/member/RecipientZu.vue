<template>
  <div class="recipku pdt pbb">
        ...
        <ul class="recipku-ul">
          <li v-for="(item,index) in userlist" :key="item.id" class="recipku-li white">
            ...
            <div class="recipku-li-cont">
              <p>备注：{{item.area}} {{item.remark}}</p>
              <p>收件人姓名：{{item.uname}}</p>
            </div>
            <div class="recipku-li-com flex-box">
              <div @click="setDefaultTap(item.id)" v-if="item.isdefault == 0" class="recipku-li-com-a">设为默认收件人组</div>
              <div v-if="item.isdefault == 1" class="recipku-li-com-a bkg">默认收件人组</div>
              <div class="recipku-li-btn flex flex-box right">
                <p @click="showConfirm(item.id)">删除</p>
                <router-link :to="'/RecipientZuEdit/'+item.id" tag="p">编辑</router-link>
              </div>
            </div>
          </li>
        </ul>
    ...
    <router-link class="submit-btn" to="/RecipientZuAdd/0" tag="div">添加</router-link>
  </div>
</template>
...
export default {
  ...
  created: function () {
    this.getList()
    this.$store.state.recipientList = []
  },
  mounted: function () {
  ...
  methods: {
    ...
    deleteTap: function () {
      this.popupVisible = false
      this.axios.get(`${this.api.delete_receipt_group}?phone=${this.$store.state.isLogin}&id=${this.userId}`).then(res => {
        console.log(res)
    ...
    setDefaultTap: function (id) {
      this.axios.get(`${this.api.set_default_group}?phone=${this.$store.state.isLogin}&id=${id}`).then(res => {
        console.log(res)
        if (res.data.code === '200') {
